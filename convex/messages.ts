...
const populateThread = async (ctx: QueryCtx, messageId: Id<'messages'>) => {
    ...
    const lastMessageUser = await popularUser(ctx, lastMessageMember.userId)
...
const popularUser = (ctx: QueryCtx, userId: Id<'users'>) => {
    return ctx.db.get(userId)
}

 const popularMember = (ctx: QueryCtx, memberId: Id<'members'>) => {
            ...
            page: (
                await Promise.all(
                    results.page.map( async (message) => {
                        const member = await popularMember(ctx, message.memberId)
                        const user = member ? await popularUser(ctx, member.userId) : null
